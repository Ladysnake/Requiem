...
public class DissolutionEntities {
    ...
    public static void init() {
        if (DebugUtil.isDevEnv()) {
            DEBUG_POSSESSABLE = Registry.register(Registry.ENTITY_TYPE, Dissolution.id("debug_possessable"), FabricEntityTypeBuilder.create(EntityCategory.MISC, PossessableEntityImpl::new).size(0.6F, 1.95F).trackable(64, 1, true).build());
