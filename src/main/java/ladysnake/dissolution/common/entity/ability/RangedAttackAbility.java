...
public class RangedAttackAbility<T extends MobEntity & RangedAttacker> extends IndirectAbilityBase<T> {
    ...
    public boolean trigger(PlayerEntity player) {
        ...
        EntityHitResult trace = ProjectileUtil.rayTrace(this.owner, startPoint, endPoint, boundingBox_1, (entity_1x) -> entity_1x != player && !entity_1x.isSpectator() && entity_1x.doesCollide(), range);
        if (trace != null) {
