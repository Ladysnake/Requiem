...
//        return FramebufferReplacement.getMainDepthTexture(); // TODO adapt this to the new rendering code
    ...
    private static void loadShaders(ResourceManager resourceManager) {
        ...
        registeredShaders.forEach((rl, sh) -> {
            try {
                ...
                Dissolution.LOGGER.error(new FormattedMessage("Could not create shader {} from vertex {} and fragment {}", rl, sh.getVertex(), sh.getFragment(), e));
            }
        });
    }
    ...
    private static int loadShader(ResourceManager resourceManager, @Nullable Identifier vertexLocation, @Nullable Identifier fragmentLocation) throws IOException {
        ...
        if (vertexLocation != null) {
            ...
            if (!log.isEmpty()) {
                Dissolution.LOGGER.error("Could not compile vertex shader " + vertexLocation + ": " + log);
            }
        }
        ...
        if (fragmentLocation != null) {
            ...
            if (!log.isEmpty()) {
                Dissolution.LOGGER.error("Could not compile fragment shader " + fragmentLocation + ": " + log);
            }
        }
        ...
        if (DebugUtil.isDevEnv()) {
            ...
            if (glGetProgrami(programId, GL_VALIDATE_STATUS) == 0) {
                Dissolution.LOGGER.warn("Warning validating Shader code:" + glGetProgramInfoLog(programId, 1024));
            }
        }
